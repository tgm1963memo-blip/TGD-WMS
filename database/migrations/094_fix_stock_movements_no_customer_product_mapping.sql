-- Migration 094: Fix tgd_create_stock_movements_from_deposit when product_id is NULL
--               and customer_product_code has no matching tgd_customer_products row.
--
-- Root cause (from diagnostic query):
--   tgd_customer_deposit_request_lines.product_id = NULL
--   tgd_customer_deposit_request_lines.customer_product_code = '10154-10' etc.
--   BUT tgd_customer_products has no row for these codes → v_cp = NULL →
--   v_internal_product_id stays NULL → CONTINUE → nothing inserted.
--
-- Fix: when v_cp IS NULL (no customer_products mapping) but customer_product_code
-- IS NOT NULL, find or create a tgd_products row using the code as SKU directly.
...
BEGIN
  ...
  LOOP
    ...
    IF v_cp IS NOT NULL THEN
      -- Path 1: found in tgd_customer_products
    ...
    ELSIF v_line.customer_product_code IS NOT NULL THEN
      -- Path 2: no tgd_customer_products row → find/create tgd_products by SKU directly
      SELECT id INTO v_internal_product_id
      FROM public.tgd_products
      ...
      IF NOT FOUND THEN
        INSERT INTO public.tgd_products (sku, name, description, created_at, updated_at)
        VALUES (
          v_line.customer_product_code,
          coalesce(v_line.product_name, v_line.customer_product_code),
          null,
          now(),
          now()
        )
    ...
    ELSIF v_line.product_id IS NOT NULL THEN
      -- Path 3: product_id set directly on the line
      v_internal_product_id := v_line.product_id;

    END IF;

    IF v_internal_product_id IS NULL THEN
      RAISE NOTICE 'Skipping line % — cannot resolve product (no customer_product_code, no product_id)', v_line.id;
      CONTINUE;
    END IF;
...
-- Re-run backfill now with fixed function
do $backfill$
declare
  v_req       record;
  v_actor_id  uuid;
begin
  ...
  loop
    raise notice 'Backfilling deposit %', v_req.id;
