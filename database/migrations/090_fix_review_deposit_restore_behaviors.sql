-- Migration 090: Fix tgd_review_customer_deposit_request
--
-- Migration 089 accidentally stripped critical behaviors from 077:
--   • ACCEPT no longer calls tgd_bridge_customer_deposit_to_receiving
--   • CONFIRM_RECEIPT no longer calls tgd_create_stock_movements_from_deposit
--     → tgd_stock_balances never written → warehouse map always shows 0%
--   • Timeline events no longer inserted
--   • reviewed_by_email, reviewed_at, last_action_by_user_id no longer updated
--
-- This migration restores all of 077's behaviors and keeps 089's additions:
--   • has_receipt_variance computed on CONFIRM_RECEIPT
--   • web_approved_by_email set on CONFIRM_RECEIPT
--   • review_comment preserved (coalesce) rather than overwritten to null
