-- Migration 082: Add temperature_type to tgd_customer_storage_rate_rules
-- Allows separate storage rates per temperature zone (Chilled / Frozen / Freeze)

begin;

-- 1. Add column (default FROZEN so existing rows remain valid)
alter table public.tgd_customer_storage_rate_rules
  add column if not exists temperature_type text not null default 'FROZEN';
...
alter table public.tgd_customer_storage_rate_rules
  ...
  check (temperature_type in ('CHILLED', 'FROZEN', 'FREEZE'));

-- 3. Replace old unique constraint (customer_id, charge_basis) with one that includes temperature_type
alter table public.tgd_customer_storage_rate_rules
  drop constraint if exists tgd_customer_storage_rate_rules_customer_basis_unique;
alter table public.tgd_customer_storage_rate_rules
  add constraint tgd_customer_storage_rate_rules_customer_basis_temp_unique
  unique (customer_id, charge_basis, temperature_type);

-- 4. Drop old function signature so we can recreate with new parameter
drop function if exists public.tgd_upsert_customer_storage_rate_rule(uuid, uuid, text, numeric, text, text, boolean);

-- 5. Recreate function with p_temperature_type parameter
...
begin
  ...
  if v_temp_type not in ('CHILLED', 'FROZEN', 'FREEZE') then
    raise exception 'temperature_type must be CHILLED, FROZEN, or FREEZE';
  end if;
