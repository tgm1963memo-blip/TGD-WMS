-- 096_fix_customer_product_real_upsert.sql
-- BUGFIX: tgd_upsert_customer_product always INSERTed when p_product_id was
-- null, despite its name. Excel import always calls it without a product_id,
-- so re-importing a code already in the catalog (or a code repeated twice in
-- one file) hit "duplicate key value violates unique constraint
-- tgd_customer_products_customer_code_unique" and aborted the whole import.
-- Fix: when p_product_id is null, look up an existing row by
-- (customer_id, customer_product_code) and UPDATE it instead of INSERTing.
...
begin
  ...
  -- No explicit product_id (the case for every Excel import row): resolve
  -- the real target by the unique (customer_id, customer_product_code) key
  -- so re-importing updates the existing row instead of colliding with it.
