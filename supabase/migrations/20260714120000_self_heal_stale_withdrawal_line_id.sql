-- Fixes "Withdrawal request line not found" hard-blocking Save on the
-- customer withdrawal draft edit page.
--
-- Root cause: this withdrawal draft (and others like it) is worked on by
-- more than one staff account over its lifetime (created_by rm2@tgm.co.th,
-- last_action_by chulikorn@tgm.co.th on CWR-20260714-0004, for example).
-- The edit page loads each line's real id once, on page open. If a
-- concurrent edit elsewhere (another tab, another staff member, or an
-- earlier partial save in the same session) deletes/renumbers that line
-- before this save runs, the browser still submits the now-stale id as
-- p_line_id. The UPDATE branch below then matched zero rows and hard-
-- raised 'Withdrawal request line not found' — which aborted the ENTIRE
-- save loop, discarding every other line's edits too, not just the stale
-- one.
--
-- Fix: only take the UPDATE branch when p_line_id still resolves to a real
-- row on this request. A stale/deleted id now falls through to the INSERT
-- branch instead of hard-failing — the line's data is preserved as a fresh
-- row (and the client picks up its new id from the response, same as any
-- other insert), instead of the whole save attempt being lost.
...
begin
  ...
  if p_line_id is not null and v_target_exists then
    ...
    -- row on this request (stale reference from a concurrent edit) — see
    -- migration header.
