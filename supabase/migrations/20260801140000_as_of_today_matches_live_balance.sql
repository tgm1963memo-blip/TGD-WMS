-- User confirmed: "ยอด ณ วันที่ปัจจุบัน" must equal the live/no-date total
-- exactly, not the stricter "physically completed by that date" figure —
-- so today specifically should read identically to leaving the date
-- blank. Only a genuinely PAST date should apply the historical
-- "confirmed/completed by then" restriction added in migrations
-- 20260801120000/20260801130000.
--
-- Implementation: p_as_of_date >= CURRENT_DATE (covers both "today" and
-- the NULL/no-filter case, since NULL already short-circuits) now
-- behaves exactly like the live path -- no confirm/complete-by-date
-- check at all, same as leaving the filter blank. Only p_as_of_date <
-- CURRENT_DATE applies the point-in-time restriction.
...
as $$
  ...
  exact_withdrawn AS (
    ...
    JOIN public.tgd_customer_withdrawal_requests wr
     ...
     AND (
       (
         (p_as_of_date IS NULL OR p_as_of_date >= CURRENT_DATE)
         AND wr.status <> 'CANCELLED'
       )
       OR (
         p_as_of_date IS NOT NULL AND p_as_of_date < CURRENT_DATE
         AND (
           EXISTS (
