-- "ในหน้าเบิก เพิ่มให้พนักงาน tgc สามารถเพิ่มรายการสินค้าในการเบิกได้
-- ในกรณีที่ lot ที่แจ้งมาสินค้าไม่เพียงพอ" — while picking a withdrawal
-- (ADMIN_ACCEPTED/WAREHOUSE_PICKING), staff can already RETAG an existing
-- line onto a different tracking code (tgd_admin_update_withdrawal_line_source,
-- migration 20260731110000) but cannot ADD a brand new line — there is no
-- withdrawal-side counterpart to
-- tgd_admin_add_customer_deposit_request_line (the deposit side's
-- "เพิ่มรายการ" tool). This matters because a shortfall usually needs a
-- SPLIT, not a swap: e.g. the notified lot only has 8 of the 20 requested
-- boxes, so staff need to keep the original 8-box line pointed at that
-- lot AND add a second line for the remaining 12 sourced from a different
-- lot — retagging the one existing line can't represent that.
--
-- Mirrors tgd_admin_add_customer_deposit_request_line's shape (role
-- check, catalog auto-fill, next line_no) but reuses the balance-lock
-- logic already written twice for withdrawal lines (
-- tgd_upsert_customer_withdrawal_request_line migration 20260731130000,
-- tgd_admin_update_withdrawal_line_source migration 20260731110000) so a
-- substitute line can't overdraw the lot it's sourced from. Allowed only
-- while the request is being actively worked (ADMIN_ACCEPTED/
-- WAREHOUSE_PICKING) — once COMPLETED/DISPATCHED/CANCELLED/REJECTED
-- there's nothing left to pick against.
...
begin
  ...
  -- Same balance-lock block used by tgd_upsert_customer_withdrawal_request_line
  -- and tgd_admin_update_withdrawal_line_source: resolve the target
  -- deposit line by tracking code (only once a specific one is picked —
  -- a bare product+lot with no tracking code, meant for FEFO
  -- auto-allocation at handheld pick time, has nothing concrete to check
  -- yet) and reject if this new line's requested amount would push total
  -- non-cancelled claims against it past what was actually deposited.
...
grant execute on function public.tgd_admin_add_customer_withdrawal_request_line(
