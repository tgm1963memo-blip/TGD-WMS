-- Fix lot-based stock matching and add admin-facing balance view.
--
-- Problem in migration 009: condition B used
--   COALESCE(wl.customer_product_code, '') = COALESCE(dl.customer_product_code, '')
-- which fails when the withdrawal line has no product_code (NULL) but the deposit
-- line does — causing the deduction to be silently skipped.
--
-- Correct rule: if withdrawal specifies a LOT, match deposit lines by that LOT.
-- Product code is optional — NULL means "any product code for this LOT".

-- ─── 1. Fix tgd_get_customer_stock_balance (customer portal) ────────────────────
...
-- ─── 2. Admin-facing balance view: all customers, role-guarded ──────────────────
