-- tgd_admin_update_withdrawal_line_source (the admin "retag" tool,
-- migration 20260729200000) was written specifically to stop staff from
-- retagging a withdrawal line onto an already-fully-claimed tracking
-- code -- but its own target-lot lookup has the same two gaps the
-- balance-lock RPC just had fixed in migration 20260731090000:
--   1. No dr.status filter, so a tracking code belonging to a recalled/
--      unconfirmed deposit (status flipped to WAREHOUSE_RECEIVING,
--      tracking_code/actual_boxes left untouched) can still be retagged
--      onto even though it isn't currently confirmed stock.
--   2. No row lock, so two concurrent admin retags onto the same
--      tracking code can each read the same "claimed by others" total
--      before either commits, both pass the check, and jointly
--      over-claim past the lot's actual quantity -- reproducing the
--      exact bad-data class this migration's own manual data-fix was
--      cleaning up.
-- Same fix pattern as 20260731090000: add the status filter and lock the
-- deposit line row for the duration of the check + retag.
