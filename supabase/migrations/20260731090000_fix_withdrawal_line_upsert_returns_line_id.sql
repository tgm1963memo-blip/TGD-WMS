-- tgd_upsert_customer_withdrawal_request_line returned 'id' = v_document.id
-- (the WITHDRAWAL REQUEST's id), but its only caller,
-- CustomerWithdrawalRequestCreatePage.jsx, reads exactly that key
-- (`lineResult.data.id`) and stores it as `line.lineId` -- expecting the
-- LINE's own id. The correct value was already being computed and
-- returned separately under 'line_id', just never surfaced under 'id'.
--
-- Same bug class as the deposit-line RPC fixed in migration
-- 20260730090000. Impact here is worse: because every line in a request
-- gets the SAME wrong value (the one shared request id), after the first
-- re-save of an existing draft every line's `lineId` collapses to the
-- request id. On the next save, the edit page's delete-diff
-- (activeLineIds vs editOriginalLineIds) can no longer tell any two lines
-- apart, so it deletes every previously-saved line by id and lets the
-- upsert-by-(withdrawal_request_id, line_no) branch recreate them all
-- with brand-new ids -- silently churning line identity on every save of
-- a multi-line withdrawal draft.
--
-- Fix: return the actual line id (already computed into v_line_id in
-- both the UPDATE and INSERT branches) under 'id' as well as 'line_id'.
-- No argument list change, so CREATE OR REPLACE keeps the existing
-- signature/grants intact.
--
-- Also closes two gaps found in the same balance-check block while
-- fixing the above (code review, not a reported incident):
--   1. The deposit-line lookup had no `for update` lock and the
--      competing-claims sum was a plain SELECT, so two concurrent saves
--      against the same tracking code could each read "0 claimed by
--      others" before either commits and both pass the balance check --
--      jointly over-claiming past the deposit line's actual quantity.
--      Locking the deposit line row serializes concurrent claims against
--      it (matches the pattern already used for v_document above).
--   2. The lookup had no dr.status filter, so a deposit recalled back to
--      WAREHOUSE_RECEIVING (status changes, tracking_code/actual_boxes
--      left untouched -- see migration 20260722120000) could still be
--      claimed against here even though tgd_get_customer_stock_balance
--      (the number shown on the customer's own balance page) already
--      excludes it. Added the same
--      dr.status in ('RECEIVED_CONFIRMED','CUSTOMER_NOTIFIED') filter
--      that balance function uses.
