-- tgd_get_customer_stock_balance / tgd_get_all_customer_stock_balances summed
-- wl.picked_boxes and wl.picked_weight independently for COMPLETED
-- withdrawals. SUM() silently skips NULL rows, so a withdrawal line
-- confirmed with only one of the two fields entered (the handheld UI allows
-- a boxes-only or weight-only pick) contributed zero to the OTHER
-- dimension's withdrawn total, even though the request is COMPLETED and the
-- goods have physically left the warehouse. Result: balance_weight (or
-- balance_boxes) kept showing stock that no longer exists, which feeds the
-- "exceeds balance" withdrawal validation, the printed remaining-balance
-- figure, and every dashboard/report built on this RPC.
--
-- Migration 20260714130000 already fixed the exact same gap in the
-- withdrawal-line balance lock (tgd_upsert_customer_withdrawal_request_line)
-- via coalesce(picked_*, requested_*) — this applies that identical pattern
-- to the two CTEs here so both balance sources agree.
