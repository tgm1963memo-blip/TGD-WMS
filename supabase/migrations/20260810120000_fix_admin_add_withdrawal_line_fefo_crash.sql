-- tgd_admin_add_customer_withdrawal_request_line's whole tracking-code
-- resolution block (including the `select ... into v_deposit_line`) only
-- ran `if v_tracking_code is not null` — leaving v_deposit_line's own
-- FEFO purpose is exactly what the "เพิ่มรายการสินค้า" (add product
-- item) modal's own hint text promises ("เว้นว่างถ้าให้ระบบเลือกล็อตให้
-- อัตโนมัติ (FEFO)" — leave blank to let the system auto-pick the lot),
-- but this RPC has never had any FEFO auto-selection logic at all: the
-- blank-tracking-code path crashed outright rather than deferring lot
-- resolution, matching how the customer-facing
-- tgd_upsert_customer_withdrawal_request_line already handles this exact
-- case (FEFO lines are inserted with source_customer_deposit_request_
-- line_id left null; the actual physical lot is picked later, at
-- handheld pick time). Reproduces for ANY blank-tracking-code add-line
-- call, for any customer/product — nothing specific to the opening-
-- balance ("OB-...") deposit line visible on screen when this was first
-- reported; that row was never even queried by the failing call.
...
begin
  ...
  if v_tracking_code is not null then
    ...
    v_source_line_id := v_deposit_line.id;
  ...
  -- v_tracking_code is null (FEFO/blank): no specific deposit lot is
  -- resolved here at all — v_source_line_id stays null, same as the
  -- customer-facing tgd_upsert_customer_withdrawal_request_line's own
  -- FEFO path. The actual physical lot is picked later, at handheld pick
  -- time (picking_rule below is set to FEFO for exactly this reason).
