-- Same bug migration 20260730090000 fixed for deposit lines, still open
-- on withdrawal lines: "ระบุกล่อง"/"ระบุน้ำหนัก" (withdrawal_qty_mode) is
-- only ever a client-side React field -- never sent to
-- tgd_upsert_customer_withdrawal_request_line, never a column on
-- tgd_customer_withdrawal_request_lines. CustomerWithdrawalLinesTable.jsx
-- already disables whichever field isn't the active mode
-- (boxesDisabled/weightDisabled) so live edits can't cross-contaminate
-- the two fields -- but reloading a saved draft has nothing to restore
-- the mode from, so customerRequestCopyUtils.js/the edit-load mapping
-- fall back to a heuristic ("BOXES if requested_boxes is filled, else
-- WEIGHT") that can guess wrong for a line that has both fields
-- populated (any WEIGHT-entered line with a whole-number box count,
-- which the box-side auto-calc happily produces). Once the mode guesses
-- wrong, the disabled/enabled sides flip, and the next edit to the
-- now-live box field auto-recomputes and silently overwrites the
-- originally precise, directly-entered weight.
--
-- Fix: persist the mode as its own column, mirroring the deposit-line
-- fix exactly.

begin;

alter table public.tgd_customer_withdrawal_request_lines
  add column if not exists pack_entry_mode text not null default 'WEIGHT'
...
drop function if exists public.tgd_upsert_customer_withdrawal_request_line(
  ...
  date, date, numeric, numeric, numeric, text, text, text
);
...
begin
  ...
  if p_line_id is not null and v_target_exists then
    ...
    insert into public.tgd_customer_withdrawal_request_lines (
      ...
      coalesce(v_pack_entry_mode, 'WEIGHT')
    )
