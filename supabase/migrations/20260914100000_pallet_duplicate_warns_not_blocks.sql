-- Turns the "this pallet already has stock on it" check from a hard block
-- into a non-blocking warning -- per explicit request: a pallet number is
-- a positional label within a row's capacity, not a strict physical lock,
-- and staff need to be able to save anyway (e.g. a mis-scanned pallet
-- number, or intentionally placing two lots on the same marked spot)
-- rather than being stuck unable to record what's physically already
-- there. The RPC now always saves the allocation and reports back whether
-- that pallet slot already had other active stock on it via a new
-- `pallet_already_in_use` field, instead of raising an exception -- the
-- caller decides how loudly to warn, but the save itself never fails for
-- this reason anymore.
...
begin
  ...
  -- Informational only now -- whether another allocation still has stock on
  -- this exact pallet slot is reported back, not enforced. boxes is null
  -- (a weight-only receipt with no box count) is conservatively always
  -- treated as active, same convention as getSectionsWithOccupancy's own
  -- occupancy math.
  select exists (
    select 1
