-- Adds a hard, server-side lock preventing a withdrawal request line from
-- ever requesting more boxes/weight than the specific deposit tracking
-- code/lot it's attributed to actually holds.
--
-- Found via a real incident: CWR-20260710-0009 requested 400 boxes against
-- tracking code FR260706002, which only ever received 100 boxes deposited
-- (CDR-20260706-0001). The client-side check (getWithdrawalBalanceInfo,
-- src/utils/customerWithdrawalLineDefaults.js) validated against the
-- POOLED total of every deposit line sharing the same lot_no (1,000 boxes
-- across three tracking codes for that lot) while the withdrawal line only
-- ever got tagged with the first matched tracking code — so the request
-- passed client validation, then printed a physically impossible pick
-- instruction for a quantity that tracking code never held. The client-side
-- fix scopes the check down to the one matched deposit line to match; this
-- migration adds the same guarantee server-side, since the RPC itself
-- previously performed no balance validation at all — a client bypass or a
-- future client bug could otherwise recreate the same incident.
--
-- Matches the deposit line the same way the client resolves it: by
-- source_customer_deposit_request_line_id when given, else by
-- tracking_code, scoped to the acting withdrawal request's own customer.
-- "Already claimed" sums requested_boxes/requested_weight from every OTHER
-- withdrawal line (any customer request, excluding the line being
-- upserted) attributed to that same deposit line, on any non-CANCELLED
-- withdrawal request — conservative on purpose: two still-in-draft
-- requests that individually look fine but would jointly overdraw a lot
-- must both be blocked before either reaches picking, not just the second
-- one to actually get picked.
...
begin
  ...
  if p_source_customer_deposit_request_line_id is not null or p_tracking_code is not null then
    ...
    if found then
      ...
      select coalesce(sum(wl.requested_boxes), 0), coalesce(sum(wl.requested_weight), 0)
