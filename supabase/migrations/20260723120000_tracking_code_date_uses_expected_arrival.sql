-- Tracking codes (FR/FZ/CH/FF/XX + date + sequence) were generated using
-- current_date -- the server's wall-clock date at the moment the customer
-- clicked Submit (or, for the ACCEPT-branch safety net, whenever admin
-- clicked Accept) -- not the deposit's own declared date. Since submission
-- can happen well before the goods actually arrive/are dated, the code's
-- embedded date drifted from "วันที่ฝากสินค้า" (the request's
-- expected_arrival_date, what the customer actually declared as the
-- deposit date and what's shown on the printed lot label).
--
-- Both generation points now use the request's own expected_arrival_date
-- instead, falling back to current_date only if that's somehow unset.
-- Forward-only: only lines assigned a tracking code from this point on are
-- affected -- no backfill of already-assigned codes.
...
begin
  ...
  select d.id, d.customer_id, d.status, d.request_no, d.expected_arrival_date into v_document
  ...
  -- rather than waiting for admin to accept. Date component is the
  -- request's own declared deposit date, not "today."
  for v_line in
