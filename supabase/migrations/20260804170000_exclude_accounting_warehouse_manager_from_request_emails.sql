-- Internal staff notification emails for a new customer deposit/withdrawal
-- request currently go to every active user in role admin, accounting,
-- warehouse_admin, or warehouse_manager. Per request: warehouse_manager
-- and accounting should stop receiving these — only admin and
-- warehouse_admin need them going forward. Only the recipient role filter
-- changes; everything else in this function (customer confirmation
-- emails, subject/body text, event branches) is copied verbatim from the
-- current definition (20260629000002_fix_withdrawal_bridge_and_review_rpcs.sql).
...
begin
  ...
  if p_notification_event = 'DEPOSIT_CONFIRMED' then
    ...
    -- Only change from the previous definition: dropped 'accounting' and
    -- 'warehouse_manager' from this role list.
    for v_recipient in
