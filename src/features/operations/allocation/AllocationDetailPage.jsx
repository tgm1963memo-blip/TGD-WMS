...
import { getWithdrawalAllocationById } from '../../../services/withdrawalAllocationService.js';
...
export function AllocationDetailPage() {
  ...
  useEffect(() => {
    ...
    getWithdrawalAllocationById(id).then(({ data, error }) => {
      if (isMounted) {
  ...
  return (
    <section className="page-shell">
      <PageHeader title="Allocation Detail" description="Read-only withdrawal allocation detail." />
      <Link className="action-link" to="/operations/allocations">Back to allocations</Link>
      <DocumentStatusCard
        title={document?.allocation_no}
        ...
        fields={[
          ...
          { label: 'Method', value: document?.allocation_method },
          { label: 'Created At', value: document?.created_at },
        ]}
      />
      <DocumentSection title="Quantity Summary">
        <QuantitySummaryCard label="Lines" value={document?.tgd_withdrawal_allocation_lines?.length ?? 0} helperText="Read-only allocation line count." />
      </DocumentSection>
      <DocumentSection title="Lines">
        <DocumentLineTable lines={document?.tgd_withdrawal_allocation_lines ?? []} columns={lineColumns} />
      </DocumentSection>
    </section>
  );
}
