...
const listOutboundDocumentsMock = vi.hoisted(() =>
  vi.fn(() => Promise.resolve([listDocument])),
);

const getOutboundDocumentDetailMock = vi.hoisted(() =>
  vi.fn(() => Promise.resolve(detailPayload)),
);

vi.mock('../../src/services/outboundPickingService.js', () => ({
  listOutboundDocuments: (...args) => listOutboundDocumentsMock(...args),
  getOutboundDocumentDetail: (...args) => getOutboundDocumentDetailMock(...args),
}));
...
async function expectDocumentNoVisible() {
  await waitFor(() => {
    expect(screen.getAllByText('OB-14J-001').length).toBeGreaterThan(0);
  });
}

beforeEach(() => {
  vi.clearAllMocks();
  listOutboundDocumentsMock.mockImplementation(() => Promise.resolve([listDocument]));
  getOutboundDocumentDetailMock.mockImplementation(() => Promise.resolve(detailPayload));
});
...
describe('Sprint 14J outbound picking read model UI', () => {
  it('renders safety note and loads outbound documents', async () => {
    ...
    expect(screen.getByText(safetyNote)).toBeInTheDocument();

    await waitFor(() => {
      expect(listOutboundDocumentsMock).toHaveBeenCalledTimes(1);
    });

    await expectDocumentNoVisible();
  });
  ...
  it('refresh/list calls listOutboundDocuments', async () => {
    ...
    await waitFor(() => {
      expect(listOutboundDocumentsMock).toHaveBeenCalledTimes(2);
    });
  });
  ...
  it('selecting a document calls getOutboundDocumentDetail and renders detail data', async () => {
    ...
    await waitFor(() => {
      expect(getOutboundDocumentDetailMock).toHaveBeenCalledWith('document-1');
    });
